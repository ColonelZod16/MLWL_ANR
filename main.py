...
# Function to plot clustering results with ellipses
def plot_clusters(X, labels, means, covariances, title):
    plt.figure(figsize=(8, 6))
    colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k']
    
    # Plot data points
    for i in range(len(means)):
        plt.scatter(X[labels == i, 0], X[labels == i, 1], s=30, c=colors[i%len(colors)], label=f'Cluster {i+1}')
    
    # Plot ellipses for Gaussian distributions
    for i, (mean, covar) in enumerate(zip(means, covariances)):
        if np.all(covar == 0):  # Skip if covariance is zero
            continue
        ...
        ell = Ellipse(xy=mean, width=width, height=height, angle=angle,
                     edgecolor=colors[i%len(colors)], fc='None', lw=2, alpha=0.5)
    ...
    # Plot base station
    plt.scatter(bs_position[0], bs_position[1], s=100, c='k', marker='*', label='Base Station')
...
plot_clusters(user_positions, kmeans_clusters, kmeans.cluster_centers_, 
             [np.eye(2)*10 for _ in range(n_clusters)], 'K-means Clustering Results')

# Traditional GMM (Figure 4)
gmm = GaussianMixture(n_components=n_clusters, random_state=42)
gmm.fit(user_positions)
gmm_clusters = gmm.predict(user_positions)
plot_clusters(user_positions, gmm_clusters, gmm.means_, gmm.covariances_, 'Traditional GMM Clustering Results')

# Improved GMM (Figure 5)
...
plot_clusters(user_positions, igmm_clusters, igmm.means_, igmm.covariances_, 'Improved GMM Clustering Results')
...
def calculate_sensing_power(W, target_angles):
    total_power = 0
    for theta in target_angles:
        a = np.exp(1j * np.pi * np.arange(N) * np.sin(theta)) / np.sqrt(N)
        for w in W:
...
# Figure 6: Convergence curves under different ρ values
# def plot_convergence_curves():
#     rho_values = [0.2, 0.5, 0.8]
#     convergence_data = {rho: [] for rho in rho_values}
    
#     for rho in rho_values:
#         W = design_noma_beamforming(igmm_cluster_channels, P_t, sigma2, r_min)
        
#         for _ in range(5):  # Number of iterations to show convergence
#             # Calculate objective function value
#             comm_rate = calculate_spectral_efficiency(igmm_cluster_channels, W, sigma2)
#             sensing_power = calculate_sensing_power(W, target_angles)
#             obj_value = rho * comm_rate + (1 - rho) * sensing_power
#             convergence_data[rho].append(obj_value)
            
#             # Update beamforming (one more iteration)
#             W = design_noma_beamforming(igmm_cluster_channels, P_t, sigma2, r_min, max_iter=1)
    
    plt.figure(figsize=(8, 5))
    for rho, values in convergence_data.items():
        plt.plot(range(1, len(values)+1), values, '-o', label=f'ρ = {rho}')
    
    plt.xlabel('Iteration')
    plt.ylabel('Objective Function Value')
    plt.title('Convergence Curves Under Different ρ Values')
    plt.grid()
    plt.legend()
    plt.tight_layout()
...
# Figure 7: Spectral Efficiency vs Transmission Power
...
plt.plot(power_levels, igmm_spectral_eff, 'b-o', label='Improved GMM NOMA-ISAC')
plt.plot(power_levels, gmm_spectral_eff, 'g--s', label='Traditional GMM NOMA-ISAC')
plt.plot(power_levels, kmeans_spectral_eff, 'r-.d', label='K-means NOMA-ISAC')
...
# Figure 8: Performance Trade-off Comparison
rho_values = np.linspace(0, 1, 11)  # From 0 (sensing only) to 1 (communication only)
igmm_comm = []
igmm_sensing = []
oma_comm = []
oma_sensing = []
...
for rho in rho_values:
    # For improved GMM NOMA-ISAC
    W = design_noma_beamforming(igmm_cluster_channels, P_t, sigma2, r_min)
    igmm_comm.append(calculate_spectral_efficiency(igmm_cluster_channels, W, sigma2))
    igmm_sensing.append(calculate_sensing_power(W, target_angles))
    
    # For OMA-ISAC
    W_oma = design_oma_beamforming(oma_cluster_channels, P_t, sigma2, r_min)
    oma_comm.append(calculate_spectral_efficiency(oma_cluster_channels, W_oma, sigma2, is_oma=True))
    oma_sensing.append(calculate_sensing_power(W_oma, target_angles))

# Normalize for plotting
igmm_comm_norm = igmm_comm / max(igmm_comm)
igmm_sensing_norm = igmm_sensing / max(igmm_sensing)
oma_comm_norm = oma_comm / max(oma_comm)
oma_sensing_norm = oma_sensing / max(oma_sensing)

plt.figure(figsize=(10, 6))
plt.plot(rho_values, igmm_comm_norm, 'b-o', label='Improved GMM Comm')
plt.plot(rho_values, igmm_sensing_norm, 'b--s', label='Improved GMM Sensing')
plt.plot(rho_values, oma_comm_norm, 'r-.d', label='OMA Comm')
plt.plot(rho_values, oma_sensing_norm, 'r:x', label='OMA Sensing')
plt.xlabel('Trade-off Factor ρ')
plt.ylabel('Normalized Performance')
plt.title('Performance Trade-off Comparison')
plt.grid()
plt.legend()
plt.tight_layout()
...
# Figure 9: Beam Strength patterns
...
plt.subplot(121)
plt.plot(np.degrees(theta_range), 10*np.log10(igmm_beam), 'b-', label='Improved GMM')
plt.plot(np.degrees(theta_range), 10*np.log10(gmm_beam), 'g--', label='Traditional GMM')
plt.plot(np.degrees(theta_range), 10*np.log10(kmeans_beam), 'r-.', label='K-means')
